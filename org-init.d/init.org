...
#+BEGIN_SRC emacs-lisp
  ...
  If ARG is non-nil, select an account from `mu4e-user-mail-address-list'."
    (interactive "P")
    (let ((address (if arg
                       (completing-read "Mail account: " mu4e-user-mail-address-list
                                        nil t nil nil user-mail-address)
                     user-mail-address)))
      (mu4e-my-set-account-dynamically address)))
#+END_SRC
...
メールアドレスに応じてアカウントを変更する関数．
#+BEGIN_SRC emacs-lisp
  (defun mu4e-my-set-account-dynamically (address)
