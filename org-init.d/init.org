...
(when window-system
  (defvar my-default-font-size 12 "Default font size")
  ...
  (defun my-resize-frame-font ()
    ...
    (catch 'end-flag
      (while t
        (setq key (aref (read-key-sequence-vector "Press +,-,0") 0))
        (cond ((= key ?+)
               (setq my-font-size (min (+ my-font-size 3) 33)))
              ((= key ?-)
               (setq my-font-size (max (- my-font-size 3) 9)))
              ((= key ?0)
