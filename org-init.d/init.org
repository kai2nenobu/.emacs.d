...
(global-set-key (kbd "H-O") 'pop-global-mark)
(global-set-key (kbd "H-I") 'pop-global-mark-reverse)
...
C-u 100 M-x increment-string-as-number ;; replaced by \"88\""
  ...
  (let ((col (current-column))
    ...
    (move-to-column col)))
(global-set-key (kbd "M-i") 'my-increment-string-as-number)
#+END_SRC
** ウィンドウを対話的にリサイズ
...
(custom-set-variables
 ...
 '(safe-local-variable-values (quote ((quickrun-option-timeout-seconds . 10) (quickrun-option-args . "") (quickrun-option-cmdopt . "") (TeX-master . "progress_report1.tex") (TeX-master . "/home/kai/Dropbox/works/tex_workspace/meeting/progress_report1.tex") (TeX-master . "bachelor_handout.tex") (TeX-master . "bachelor_thesis.tex") (TeX-master . t)))))
(custom-set-faces
...
(my-safe-require 'mu4e
  ...
  (setq mu4e-html2text-command "html2text -utf8 -width 72")  ;; html をテキスト表示
...
#+BEGIN_SRC emacs-lisp
  (defun mu4e-my-next-folder (arg)
    "Move to a next folder."
    (interactive "p")
    (let* ((mdir (mu4e-field-at-point :maildir))
  ...
  (defun mu4e-my-previous-folder (arg)
    "Move to a previous folder."
    (interactive "p")
    (mu4e-my-next-folder (- arg)))

  (define-key mu4e-headers-mode-map (kbd "M-n") 'mu4e-my-next-folder)
  (define-key mu4e-headers-mode-map (kbd "M-p") 'mu4e-my-previous-folder)
  (define-key mu4e-view-mode-map (kbd "M-n") 'mu4e-my-next-folder)
  (define-key mu4e-view-mode-map (kbd "M-p") 'mu4e-my-previous-folder)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun helm-mu4e-maildir ()
    ...
    (helm :sources   'helm-c-source-mu4e-maildir
              ...
              :preselect (let ((msg (mu4e-message-at-point t)))
                           (and msg (mu4e-msg-field msg :maildir)))))
...
(my-safe-require 'guide-key
  (setq-default guide-key/guide-key-sequence
                '("C-x v" "C-c p" "C-x RET" "C-x 8" "C-x 8 ^"
                  ...
                  "<key-chord> : h" "<key-chord> h :" "<key-chord> : p" "<key-chord> p :"))
...
helm-c-apropos は複数の情報源でできているので，それぞれの persistent-action
を変更する．helm-c-describe-face が定義されていなかったので，自分で書いた．
#+BEGIN_SRC emacs-lisp
  (defadvice helm-c-source-emacs-commands (after change-persisten-action activate)
    ...
    (let ((source ad-return-value)
      ...
      (setcdr (assoc 'persistent-action source) action)
      (setcdr (assoc 'persistent-help source) help)
...
(my-safe-require 'viewer
  ...
  (setq viewer-modeline-color-root "magenta")
