...
From アドレスを変更することができた．`user-mail-address' を直接設定しているの
で注意．新規作成時には元のメールがないので，どうやって From アドレスを設定す
るか悩みどころ．
#+BEGIN_SRC emacs-lisp
(defun mu4e-my-set-from-address ()
  "Set the From address based on the To address of the original."
  (when mu4e-compose-parent-message
    (let (mdir)
      (setq mdir (mu4e-message-field mu4e-compose-parent-message :maildir))
      (setq user-mail-address
            (cond ((string-match "^/gavo/" mdir) "kai@gavo.t.u-tokyo.ac.jp")
                  ((string-match "^/kbkbkbkb1/" mdir) "kbkbkbkb1@gmail.com")
                  ((string-match "^/cellphone/" mdir) "giovanni-franz@ezweb.ne.jp"))
            ))))
(add-hook 'mu4e-compose-pre-hook 'mu4e-my-set-from-address)
#+END_SRC

From だけじゃなくて，smtp サーバー設定なんかも一緒に変えたほうがいいのかも．
#+BEGIN_EXAMPLE
