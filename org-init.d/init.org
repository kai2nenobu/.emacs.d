...
(let* ((choco-dir (getenv "ChocolateyToolsLocation"))
       ...
       (cond ((file-exists-p git-for-win-dir)
         (add-to-list 'exec-path (expand-file-name "mingw64/bin" git-for-win-dir))
...
(define-key my-original-map (kbd "C-l") (if (>= emacs-major-version 26)
...
(use-package browse-url :defer t
  ...
  (defun my-switch-browser ()
    ...
    (let* ((browser-alist '(("default" . browse-url-default-browser)
                            ...
                            ;; TODO: no browser function for IE
                            ("IE" . browse-url-*)
                            ("W3" . browse-url-w3)))
  ...
  (let* ((pf (or (getenv "PROGRAMFILES(x86)") (getenv "PROGRAMFILES")))
         (firefox (expand-file-name "Mozilla Firefox/firefox.exe" pf))
         (chrome (expand-file-name "Google/Chrome/Application/chrome.exe" pf)))
    (when (file-exists-p firefox)
      (message "Use Firefox at %s" firefox)
      (set 'browse-url-firefox-program firefox))
    (when (file-exists-p chrome)
...
(use-package desktop
  ...
  (set-variable 'desktop-restore-frames nil)
