               ...
               (t
                (let ((word (thing-at-point 'word)))
                  (read-string (concat "Look up word"
                                       (if word (concat " (default " word ")") "")
                                       ": ")
                               nil nil word t))))))
  (let ((alc-url "http://eow.alc.co.jp/%s/UTF-8/"))
    (browse-url (format alc-url (my-url-encode-string query)))))

(defun my-url-encode-string (string)
  (let ((command "nkf -wMQ | sed 's/=$//' | tr = % | tr -d '\n'"))
    (with-temp-buffer
      (insert string)
...
org-init.d/init.org を更新したら，明示的に init.el を更新したい時があるので作っ
た．
#+BEGIN_SRC emacs-lisp
(defun my-update-org-init ()
  (interactive)
  (org-babel-tangle-file (expand-file-name "init.org" org-init-directory)
                         (expand-file-name "init.el" org-init-directory)
                         "emacs-lisp"))
#+END_SRC
** 順方向に mark-ring をたどる
...
*** Windows
#+BEGIN_SRC emacs-lisp
(when (and (winp) (require 'w32-ime nil t))
...
** zencoding-mode.el
#+BEGIN_SRC emacs-lisp
(my-safe-require 'zencoding-mode
...
(my-safe-require 'web-mode
  ...
  (defun my-web-mode-initial-customization ()
    ;; (font-lock-mode -1) にしたいけど，このあとに font-lock-mode を有効にする
    ;; フックが実行されてしまうようだ．苦肉の策で run-with-timer で遅延させる
    (run-with-timer 1 nil 'font-lock-mode '(-1)))

  (add-hook 'web-mode-hook 'zencoding-mode)

  ;(add-hook 'web-mode-hook 'my-web-mode-initial-customization)
  )
#+END_SRC
** mu4e.el                                                         :noexport:
新しいメーラー．
...
(my-safe-require 'guide-key
  ...
  (defun guide-key/my-hook-function-for-web-mode ()
    (dolist (key '("C-c"))
      (guide-key/add-local-guide-key-sequence key))
    (guide-key/add-local-highlight-command-regexp "web-mode-"))
  (add-hook 'web-mode-hook 'guide-key/my-hook-function-for-web-mode)
  )
#+END_SRC
...
auto-sve-buffersの拡張版。アイドル時にファイルを自動保存する。
#+BEGIN_SRC emacs-lisp
(my-safe-require 'auto-save-buffers-enhanced nil t)
...
#+END_SRC
*** magit のファイル名をマークする
- [[http://d.hatena.ne.jp/ken_m/20111225/1324833439][Magit で複数のファイルに対して操作する - ”E” for ...]]
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq org-src-preserve-indentation t)  ; ソースブロックのインデントを保持する．エクスポート時も
