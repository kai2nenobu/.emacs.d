...
;; `mode-line-mule-info' をコピーして文字エンコーディングの
;; ニーモニック部分を差し替える（リストの3番目）
(defvar my-mode-line-mule-info (copy-sequence mode-line-mule-info)
  "Modified version of `mode-line-mule-info'")
(put 'my-mode-line-mule-info 'risky-local-variable t)
(setf (nth 2 my-mode-line-mule-info) '(:eval (my-buffer-coding-system-mnemonic)))

;; `mode-line-format' の `mode-line-mule-info' を独自のものに差し替える
(setq-default mode-line-format
              (substitute 'my-mode-line-mule-info 'mode-line-mule-info mode-line-format))
#+END_SRC
...
(use-package hiwin
  ...
  (setq hiwin-always-active-buffer-name-regexp
        (concat hiwin-always-active-buffer-name-regexp "\\|^\\*magit\\|\\*PLANTUML Preview\\*"))
