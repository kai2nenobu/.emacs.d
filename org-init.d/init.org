...
(let* ((choco-dir (getenv "ChocolateyToolsLocation"))
       (git-for-win-dir (expand-file-name "Git"
                                         (or (getenv "ProgramW6432") (getenv "Program"))))
  ...
  (cond ((file-exists-p git-for-win-dir)
         (add-to-list 'exec-path (expand-file-name "bin" git-for-win-dir))
         (add-to-list 'exec-path (expand-file-name "usr/bin" git-for-win-dir)))
        ((file-exists-p cmder-dir)
         (add-to-list 'exec-path (expand-file-name "vendor/git-for-windows/mingw32/bin" cmder-dir))
         (add-to-list 'exec-path (expand-file-name "vendor/git-for-windows/cmd" cmder-dir))
         (add-to-list 'exec-path (expand-file-name "vendor/git-for-windows/usr/bin" cmder-dir)))
...
(defvar my-mode-line-coding-format
      '(:eval
        (let* ((code buffer-file-coding-system)
               ...
               (eol (coding-system-eol-type-mnemonic code)))
          (format "%s%s%s" name bom eol))))
...
#+BEGIN_SRC emacs-lisp +n
(let ((loaddefs (locate-user-emacs-file "my-lisp/loaddefs.el")))
  (when (file-exists-p loaddefs)
    (load loaddefs 'noerror 'nomsg)))
#+END_SRC
** uiflow向けのメジャーモード
...
#+END_SRC
*** textlintで文章校正
事前にnpmで =textlint= と好きなルールをグローバルにインストールしておく。
#+BEGIN_EXAMPLE
$ npm install -g textlint textlint-rule-preset-jtf-style
#+END_EXAMPLE

textlintを使ったチェッカーを定義する
#+BEGIN_SRC emacs-lisp +n
(with-eval-after-load 'flycheck
  (when (executable-find "textlint")
    (flycheck-define-checker textlint
      "A linter by textlint."
      :command ("textlint" "--format" "unix" "--preset" "jtf-style" source-inplace)
      :error-patterns
...
(use-package hiwin
  ...
  (setq hiwin-always-active-buffer-name-regexp
        (concat hiwin-always-active-buffer-name-regexp "\\|^\\*magit"))
...
\n(git commit --amend --no-edit ARGS)"
  ...
  ;; git log に --first-parent オプションを指定できるようにする
  (let ((switches (plist-get magit-log-popup :switches))
        (new-switch (?f "Show only the first parent commit" "--first-parent")))
    (plist-put magit-log-popup :switches (cons 'new-switch switches)))
#+END_SRC
*** キーバインド
#+BEGIN_SRC emacs-lisp +n
