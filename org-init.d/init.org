...
#+BEGIN_SRC emacs-lisp
(defun my-open-file-with-external-app  (file)
...
(defun my-open-file-dwim ()
  ...
  (let ((file-list
    ...
    (when (or (<= (length file-list) 5)
      ...
      (mapc 'my-open-file-with-external-app file-list))))
(add-hook 'dired-mode-hook
          (lambda ()
            (define-key dired-mode-map "z" 'my-open-file-dwim))) ;;; 関連付け
(define-key my-original-map (kbd "C-o") 'my-open-file-dwim)
#+END_SRC
*** スペースでマークする (FD like)
#+BEGIN_SRC emacs-lisp
...
(my-safe-require 'direx
  ...
  ;; アイコンに画像を使う
  ;(let ((dir "~/local/share/emacs/24.2/etc/images"))
  ;  (setq direx:leaf-icon
  ;        (propertize "   "
  ;                    'display (create-image (expand-file-name "tree-widget/folder/leaf.png" dir))
  ;                    'help-echo "Open file")
  ;        direx:closed-icon
  ;        (propertize "   "
  ;                    'display (create-image (expand-file-name "tree-widget/folder/close.png" dir))
  ;                    'help-echo "Open folder")
  ;        direx:open-icon
  ;        (propertize "   "
  ;                    'display (create-image (expand-file-name "tree-widget/folder/open.png" dir))
  ;                    'help-echo "Close folder")))
  ;; アイコンの設定
  (setq direx:leaf-icon   "├ "
        direx:closed-icon "▶ "
        direx:open-icon   "▼ ")
