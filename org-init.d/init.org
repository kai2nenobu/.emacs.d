...
#+BEGIN_SRC emacs-lisp
  ...
  ;; export 時にソースコードブロックの文末に変な文字が入るのを防ぐ
  ;; 非常にハードなハックだけど，とりあえずしょうがない
  (defun org-export-my-delete-trail ()
    (goto-char (point-min))
    (while (re-search-forward "\\(?:&#5734[45];\\)\\{1,3\\}$" nil t)
      (replace-match "")))
  (add-hook 'org-export-html-final-hook 'org-export-my-delete-trail)
  ...
  (setq org-publish-project-alist
        `(("html"
          ...
          ("kensyu-html"
           ...
           :style "<link rel=stylesheet type=\"text/css\" href=\"/~kai/stylesheet.css\" >"
           :html-preamble t
           :html-postamble "[<a href=\"/~kai/index.html\">トップへ戻る</a>] [<a href=\"javascript:history.go(-1)\">前に戻る</a>]"
           :exclude "TODO.org"
    ...
    (let ((case-fold-search nil)
      ...
      (concat "^"
              ...
              (replace-regexp-in-string "[A-Z]"
                                        'translate-camel-regexp str t nil nil 1))))

  (defun translate-camel-regexp (string)
    (concat "[a-z]*[-:/]" (downcase string)))

  (defun camel-all-completion (string collection)
    ""
    (let ((regexp (camel-regexp string)))
      (loop for str in collection
            if (string-match regexp str)
            collect str)))

  (dolist (source '(ac-source-variables ac-source-features ac-source-functions ac-source-symbols))
    (add-to-list source (cons 'match 'camel-all-completion)))
