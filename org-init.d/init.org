...
(defun org-my-edit-src-exit (&optional context)
  ...
  (when (and (buffer-modified-p)
    ...
    (let ((org-buf (marker-buffer org-my-edit-src-marker))
      ...
      (goto-char (point-min))
      (re-search-forward (concat block-sep "\n") nil t)
      (setq org-pt (marker-position org-my-edit-src-marker))
      (dolist (info org-my-edit-src-info-list)
        (setq block-beg (point)
              block-end (progn (re-search-forward (concat block-sep "\n") nil t)
                               (match-beginning 0))
...
#+END_SRC
**** 参考URL
- 導入の仕方など [[http://mitukiii.jp/2010/10/29/install-rsense-to-mac/][mitukiii.jp | Emacs/Vimなどに特化したRubyのための開発補助ツー
  ルRsenseをMac に入れてみた]]
** 標準ライブラリ
#+BEGIN_SRC emacs-lisp
(my-safe-require 'cl)
