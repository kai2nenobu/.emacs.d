...
anything で mu4e のフォルダを選択する．
#+BEGIN_SRC emacs-lisp
  (defvar anything-c-source-mu4e-maildir
    '((name . "mu4e-maildir")
      ...
      (migemo))
    "Anything source for mu4e maildir.")

  (defun anything-mu4e-maildir ()
    "Anything command for mu4e maildir."
    (interactive)
    (anything :sources   'anything-c-source-mu4e-maildir
              :buffer    "*anything-mu4e-maildir*"
              :preselect (let ((msg (mu4e-message-at-point t)))
                           (and msg (mu4e-msg-field msg :maildir)))))
  (define-key mu4e-main-mode-map (kbd "f") 'anything-mu4e-maildir)   ; smtpmail-send-queued-mail を上書き
  (define-key mu4e-headers-mode-map (kbd "f") 'anything-mu4e-maildir)
  (define-key mu4e-view-mode-map (kbd "f") 'anything-mu4e-maildir)
#+END_SRC

anything でメールアドレスの補完をする．メールアドレスは
...
#+BEGIN_SRC emacs-lisp
  ...
  (defvar anything-c-source-mu4e-my-contact-history
    '((name . "mu4e Contact History")
      (candidates . mu4e-my-contact-history)
      (action . mu4e-my-insert-contact-and-save-history)
      (migemo))
    "Anything source for mu4e contact history.")

  (defvar anything-c-source-mu4e-contact
    '((name . "mu4e Contact Completion")
      (candidates . mu4e~contacts-for-completion)
      (action . mu4e-my-insert-contact-and-save-history)
      (migemo))
    "Anything source for mu4e contact.")

  (defun anything-mu4e-contact ()
    "Anything command for mu4e contact."
    (interactive)
    (anything-other-buffer '(anything-c-source-mu4e-my-contact-history
                             anything-c-source-mu4e-contact)
                           "*anything-mu4e-conntact*"))
  (define-key mu4e-compose-mode-map (kbd "C-c c") 'anything-mu4e-contact)
#+END_SRC
