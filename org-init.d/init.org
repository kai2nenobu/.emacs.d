...
* Question                                                :noexport:Question:
** ミニバッファ中に anything を発動させるには？
ミニバッファの再帰的な利用法を知りたい．
** なんか anything のバッファが bottom に固定表示される？
なんで？　アップデートしたからか．
*** Answer
popwin に anything の設定が追加されているだけだった．popwin の設定を削除した．
...
(defconst my-individual-elisp-directory
  (list (locate-user-emacs-file "my-lisp"))
...
;; site-lisp 以下を追加
(setq load-path
      (delete (locate-user-emacs-file "site-lisp/org-mode")
              load-path))
(add-to-list 'load-path
             (locate-user-emacs-file "site-lisp/org-mode/contrib/lisp"))
...
になったらしい。しかし24.2でも実際は =default-major-mode= が使われるっぽい？
の値を =nil= にしておけば =major-mode= をみてくれるようになった。ちなみに
...
(defun toggle-frame-alpha ()
  "Toggle the value of alpha in `default-frame-alist'"
  (interactive)
  (let* ((alpha-cell (assq 'alpha default-frame-alist))
         (alpha (cdr alpha-cell)))
    (if alpha-cell
        (setcdr alpha-cell (if (and (numberp alpha) (= alpha default-alpha))
                          1.0 default-alpha))
      (add-to-list 'default-frame-alist (cons 'alpha default-alpha)))
...
Leave one space or none, according to the context."
  ...
  (save-excursion
    ...
    (if (or (looking-at "^\\|\\s)")
            ...
            (string-match "、\\|。\\|，\\|．" (char-to-string (preceding-char)))
            (string-match "、\\|。\\|，\\|．" (char-to-string (following-char)))
            (string-match "！\\|？" (char-to-string (following-char)))
            )
        nil
      (insert ?\s))))
...
*** Windows
#+BEGIN_SRC emacs-lisp
(when (and (winp) (fboundp 'w32-ime-initialize))
...
VBの編集用メジャーモード。
#+BEGIN_SRC emacs-lisp
(dolist (suffix '("\\.vb\\'" "\\.vbs\\'" "\\.dcm\\'" "\\.bas\\'" "\\.frm\\'" "\\.cls\\'"))
  (add-to-list 'auto-mode-alist `(,suffix . visual-basic-mode)))
...
** ox-pandoc
#+BEGIN_SRC emacs-lisp
(my-safe-require 'ox-pandoc)
#+END_SRC
** powershell-mode
#+BEGIN_SRC emacs-lisp
(my-safe-require 'powershell-mode
...
#+BEGIN_SRC emacs-lisp
(if (not (winp))
  (my-safe-require 'exec-path-from-shell
      (exec-path-from-shell-initialize)))
#+END_SRC
** groovy-mode
#+BEGIN_SRC emacs-lisp
(my-safe-require 'groovy-mode
...
** flyckeck
#+BEGIN_SRC emacs-lisp
(my-safe-require 'flycheck
  (add-hook 'prog-mode-hook 'flycheck-mode)
  (global-set-key (kbd "C-,") 'flycheck-previous-error)
  (global-set-key (kbd "C-.") 'flycheck-next-error)
  (setq flycheck-idle-change-delay 0.5)
  (setq flycheck-display-errors-delay 0.1)
  (my-safe-require 'flycheck-pos-tip
    (setq flycheck-display-errors-function 'flycheck-pos-tip-error-messages))
  )
#+END_SRC
** git-gutter-fringe
#+BEGIN_SRC emacs-lisp
(my-safe-require 'git-gutter-fringe
...
スマートに改行を入れる。
#+BEGIN_SRC emacs-lisp
;(my-safe-require 'smart-newline
;  (define-global-minor-mode global-smart-newline-mode
;    smart-newline-mode smart-newline-mode)
;  (global-smart-newline-mode 1))
#+END_SRC
** ace-jump-mode
#+BEGIN_SRC emacs-lisp
(my-safe-require 'ace-jump-mode
...
(my-safe-require 'org-octopress
  ...
  (setq org-octopress-setup-file          "~/Dropbox/octopress/org-octopress/style.org")
...
#+BEGIN_SRC emacs-lisp
(my-safe-require 'outline-magic
  (define-key outline-minor-mode-map (kbd "<C-tab>") 'outline-cycle))
#+END_SRC
** anzu
#+BEGIN_SRC emacs-lisp
(my-safe-require 'anzu
  (global-anzu-mode)
...
#+BEGIN_SRC emacs-lisp
(my-safe-require 'highlight-symbol
...
#+BEGIN_SRC emacs-lisp
(my-safe-require 'direx
  (my-safe-require 'direx-project)
  ;; アイコンに画像を使う
...
** multiple-cursor
#+BEGIN_SRC emacs-lisp
(my-safe-require 'multiple-cursors
...
合してしまうようだ．
#+BEGIN_SRC emacs-lisp
(my-safe-require 'web-mode
  (dolist (ext '("\\.phtml\\'" "\\.tpl\\.php\\'" "\\.jsp\\'" "\\.as[cp]x\\'"
...
(my-safe-require 'mu4e
  ...
  (setq message-signature "東京大学情報理工学系研究科電子情報学専攻
峯松研究室 修士2年 甲斐常伸
E-mail: kai@gavo.t.u-tokyo.ac.jp")
  ;; guide-key を使う
...
#+BEGIN_SRC emacs-lisp
(my-safe-require 'hl-line+   ; hl-line is also loaded
...
#+BEGIN_SRC emacs-lisp
(my-safe-require 'auto-save-buffers-enhanced
...
#+END_SRC
** egg.el
- [[https://github.com/bogolisk/egg][bogolisk/egg]]
git クライアント．
#+BEGIN_SRC emacs-lisp
;(my-safe-require 'egg)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
;;;; yasnippet.el
(lazyload (yas-expand yas-minor-mode helm-c-yas-complete) "yasnippet"
...
;;; yasnippet を anything で操作
  (eval-after-load "helm"
    '(progn
       (my-safe-require 'helm-c-yasnippet
         (setq helm-c-yas-space-match-any-greedy t) ; スペース区切りで絞り込めるようにする デフォルトは nil
         (define-key mode-specific-map (kbd "y") 'helm-c-yas-complete)
         )))
  )
#+END_SRC
** cygwin-mount.el
...
#+BEGIN_SRC emacs-lisp
(my-safe-require 'helm-config
  (helm-mode 1)
  (when (executable-find "cmigemo")
    (my-safe-require 'helm-migemo)
  ...
  (my-safe-require 'helm-descbinds
...
* 外部プログラムが必要そうなライブラリ
:PROPERTIES:
:ARCHIVE:  %s_archive::* 外部プログラムが必要そうなライブラリ
:END:
ポータブルにはしにくそうなやつ
** edbi.el
#+BEGIN_SRC emacs-lisp
...
(when (executable-find "cmigemo")
  (my-safe-require 'migemo
    ...
    (cond ((linuxp)
          ...
          (t
           (setq migemo-dictionary (expand-file-name "dict/utf-8/migemo-dict"
...
* 環境specificな設定を読み込む
#+BEGIN_SRC emacs-lisp
(when (file-exists-p (expand-file-name "internal.el" user-emacs-directory))
  (load (expand-file-name "internal.el" user-emacs-directory)))
#+END_SRC
