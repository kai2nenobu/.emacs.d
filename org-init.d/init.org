...
(use-package info :defer t
  ...
  ;; Cygwinのinfoを追加する
  (when (and (winp) (file-exists-p "/usr/share/info"))
...
(use-package moz :defer t
  :commands (moz-send-line moz-scroll-down moz-scroll-up moz-prev-tab moz-page-reload)
  :init
...
名前のついたブロックの内容をそのまま参照するdynamic block。
#+BEGIN_SRC emacs-lisp +n
  (defun org-dblock-write:reference-block (params)
    ...
    (let* ((ref-name (plist-get params :ref-name))
           ...
           (block (save-excursion (goto-char block-pos) (org-element-at-point))))
    ...
    (let* ((link-plist (nth 0 (cdr (org-find-text-property-in-string :parent data))))
      ...
      (when (and (eq back-end 'html)
        ...
        (let* ((name (match-string 1 link-path))
               ...
               (man-root (assoc-default name info-link-alist))
