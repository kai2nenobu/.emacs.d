...
;; site-lisp 以下を追加
(setq load-path
      (delete (expand-file-name "site-lisp/org-mode" user-emacs-directory)
              load-path))
...
(defun package-sync-cask ()
  ...
  (interactive)
  (with-temp-buffer
    (insert-file-contents-literally (expand-file-name "Cask" user-emacs-directory))
    (goto-char (point-min))
    (let (packages)
...
(my-safe-require 'helm-config
  ...
  (add-to-list 'helm-c-source-recentf '(migemo)) ; migemo 化
  (add-to-list 'helm-c-source-buffers-list '(migemo))
