...
(my-safe-require 'helm-config
  ...
  (define-key helm-command-map (kbd "g") 'helm-grep)
  (define-key helm-command-map (kbd "R") 'helm-resume)
  (global-set-key (kbd "C-S-a") 'helm-command-map)
  (eval-after-load "key-chord"
    '(progn
       (defalias 'helm-command-prefix helm-command-map)
       (key-chord-define-global (kbd ":a") 'helm-command-prefix)))
  (add-to-list 'helm-c-source-recentf '(migemo)) ; migemo 化
  (add-to-list 'helm-c-source-buffers+ '(migemo))
