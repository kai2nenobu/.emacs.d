...
(with-eval-after-load 'irfc
  ...
  (defvar helm-source-irfc
    '((name . "RFC List")
      (candidates . helm-irfc--candidates)
  ...
  (defun helm-irfc-visit (rfc)
    (let ((doc-id (plist-get rfc :doc-id)))
      (irfc-visit (substring doc-id 3))))

  (defun helm-irfc--candidates ()
    "Return a list of RFC from a index xml.
You should download the index xml (named rfc-index.xml) into `irfc-directory'
from \"http://www.ietf.org/rfc/rfc-index.xml\"."
    (with-temp-buffer
      (insert-file-contents-literally (expand-file-name "rfc-index.xml" irfc-directory))
      (goto-char (point-min))
      (let ((doc-id-regexp "^        <doc-id>\\(RFC[0-9]+\\)</doc-id>")
        ...
        (while (re-search-forward doc-id-regexp nil t)
          (let ((doc-id (match-string 1)))
            ...
            (push (list (incf i) ; cdrが渡されても大丈夫なようにリストの先頭にインデックスを加えておく
                        :doc-id doc-id
