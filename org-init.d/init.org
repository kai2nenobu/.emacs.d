...
- [[https://chocolatey.org/packages/Emacs][Chocolatey Gallery | Emacs 26.1.0.20180701]]
  - 公式バイナリをインストールするパッケージ。chocolateyでインストールできるので楽ちん。バージョン26.1の公式バイナリならほぼ emacs-w64 と機能的に差はなさそう。
* 最初に必要な設定
...
(defun my-locate-user-tmp-file (name)
  ...
  (let ((tmp-dir (locate-user-emacs-file "tmp")))
    (expand-file-name name tmp-dir)))
;; Ensure tmp directory exists
(make-directory (my-locate-user-tmp-file ".") :parents)
#+END_SRC
** ユーティリティ
#+BEGIN_SRC emacs-lisp +n
...
#+END_SRC
** デフォルト無効のコマンドを有効にする
使用頻度の低いコマンドはデフォルトで無効になっているものがある。例えば ~narrow-to-region~ などである。自分的には使いたいコマンドを有効にする。
...
#+BEGIN_SRC emacs-lisp +n
;; 記号類にデフォルトフォントを使わない（fontsetの設定を尊重する）
...
(when window-system
  ...
  (defvar my-font-alist '(("HackGen" . 13) ("Ricty" . 15) ("Ricty Diminished" . 15) ("IPAGothic" . 15) ("ＭＳ ゴシック" . 12))
  ...
  ;; 個別設定
  (set-fontset-font t (cons #x2500 #x257f) "HackGen Console") ; 罫線類を半角にする
  (set-fontset-font t (cons #xe000 #xf8ff) "HackGenNerd") ; Nerd Font（私用領域）
  )
...
- [[http://misohena.jp/blog/2017-09-26-symbol-font-settings-for-emacs25.html][Emacs 25.2以降でASCIIと日本語のフォントを別々に設定する(一部の記号のフォントが変更できない場合)]]
  - ~use-default-font-for-symbols~ を ~nil~ に設定しないと記号のフォントが変わらない話。
** default-major-mode
...
*** WSLのemacslientで開いたウィンドウにフォーカスを当てる
~emacsclient~ でファイルを開いたときに発火するフックが ~server-visit-hook~ になるので、このタイミングで ~AutoHotkey~ を使ってX Serverのウィンドウにフォーカスを当てるようにする。この方法ならコンソールから ~emacsclient~ を起動した場合でも、右クリックで ~emacsclient~ を起動した場合でもフォーカスが当てられる。

#+begin_src emacs-lisp +n
(when (and (wslp) (executable-find "AutoHotkey.exe"))
  (defun my-focus-xwindow-by-ahk ()
    "Focus to a X Server Window by AutoHotkey."
    (when (and (wslp) (executable-find "AutoHotkey.exe"))
      (let* ((userprofile (getenv "USERPROFILE"))
             (ahk-script (wslpath (expand-file-name "utils/focus-wsl-emacs.ahk" userprofile) t 'windows)))
        ;; Execute an autohotkey script
        (call-process "AutoHotkey.exe" nil nil nil ahk-script))))
  (add-hook 'server-visit-hook #'my-focus-xwindow-by-ahk)
  )
#+end_src
...
#+END_SRC
* 自作関数
** ファイル名のHOME部分を ~/ で省略する関数
何かしらのファイルリストを返す関数に対して ~:filter-return~ アドバイスすると便利です。

#+begin_src emacs-lisp +n
(defun my-abbreviate-files (files)
  "Return FILES which name is abbreviated.

This function is supposed to use with :filter-return advice."
  (mapcar #'abbreviate-file-name files))
#+end_src
** org-codeを分かち書きする関数
#+begin_src emacs-lisp +n
(defun my-wakati-org-code ()
  "Divide words for `org-code' syntax.

FIXME: コード部に日本語が含まれる場合に誤ってスペースを入れてしまう問題あり。
"
  (interactive)
  (save-excursion
    (goto-char (point-min))
    ;; front =
    (while (re-search-forward "\\([[:multibyte:]]\\)\\(=[^=\n]*=\\)" nil t)
      (replace-match "\\1 \\2"))
    (goto-char (point-min))
    ;; back =
...
#+end_src
** 新規作成ファイルのエンコーディングを変更する
新規作成ファイルのデフォルトの文字コードはUTF-8、改行文字はプラットフォーム依存にしてあるが、特定のファイルは固定のエンコーディングにしたい場合がある。たとえばps1とかbatなどである。

~file-coding-system-alist~ がそういう用途に使えるかと思ったけど、これを設定するとファイルの新規作成時だけではなく、既存のファイルを開いた際にも強制的に設定したエンコーディングになってしまう。そうすると共同プロジェクトでは不都合な場合もある。

今回はあくまでファイル新規作成時のみ設定したエンコーディングにしたいので自作してみた。 ~find-file-not-found-functions~ が存在しないファイルを開いたときに実行されるフックである。

#+begin_src emacs-lisp +n
(defun my-new-file-encoding ()
  "Change the file encoding system according to `buffer-file-name'. Always return nil."
  (when buffer-file-name
    (let ((coding-alist '(("\\.ps1\\'" . utf-8-with-signature-dos)
                          ("\\.bat\\'" . cp932-dos)))
          (match nil)
          (new-cs buffer-file-coding-system))
      (while (and (not match) coding-alist)
        (let ((pair (pop coding-alist)))
          ;; Find a first match pair
          (when (string-match (car pair) buffer-file-name)
            (setq match pair)
            (set-buffer-file-coding-system (cdr pair) nil t)))))))
(add-hook 'find-file-not-found-functions #'my-new-file-encoding)
#+end_src

参考

- [[http://yohshiy.blog.fc2.com/blog-entry-273.html?sp][Emacs における日本語文字コードの設定]]
- [[https://www.gnu.org/software/emacs/manual/html_node/emacs/Recognize-Coding.html][22.6 Recognizing Coding Systems]]
** フォントプレビュー
様々なフォントファミリーで指定した文字列をプレビューするコマンド。

- [[https://emacs.stackexchange.com/a/15137]]

をパクりました。

#+begin_src emacs-lisp +n
(defun my-font-preview (str)
  "List STR with available fonts."
  (interactive "MInput: ")
  (let ((buffer-name "*font families*"))
    (with-current-buffer (get-buffer-create buffer-name)
      (font-lock-mode 0)
      (erase-buffer)
      (dolist (font-family (-uniq (font-family-list)))
        (insert (propertize str 'face `(:family ,font-family))
                "\t"
                font-family
                "\n"))
      (goto-char (point-min)))
...
** wslpathラッパー
#+begin_src emacs-lisp +n
(defun wslpath (path &optional absolute type)
  "Translate PATH with `wslpath' command.

If ABSOLUTE is non-nil, translate to absolute path format.
TYPE is a type of path format. TYPE must be one of the following symbols.
  unix - translate from a Windows path to a WSL path (default)
  windows - translate from a WSL path to a Windows path
  mixed - translate from a WSL path to a Windows path, with '/' instead of '\\'
"
  (unless path (error "`path' must not be nil."))
  (unless type (setq type 'unix))
  (unless (member type '(unix windows mixed)) (error "Invalid `type'."))
  (let ((type-alist '((unix . "-u")
                      (windows . "-w")
                      (mixed . "-m")))
        (args (list path)))
    (add-to-list 'args (alist-get type type-alist))
    (when absolute (add-to-list 'args "-a"))
    (string-trim-right (with-temp-buffer
                         (apply 'call-process "wslpath" nil t nil args)
                         (buffer-string))
                       "\r?\n" ; trim a trailing newline
                       )))
#+end_src
** prettierでフォーマット修正
prettierを呼び出すだけの関数。yamlとか書いてるときに便利。

#+BEGIN_SRC emacs-lisp +n
(defun my-prettier-write ()
  "Fix bad formats in current buffer by prettier."
  (interactive)
  (let (exit-status
        (file (buffer-file-name)))
    (with-temp-buffer
      (setq exit-status (call-process "npx" nil t t "prettier" "--write" file))
      (goto-char (point-max))
      (insert (format "prettier exited with %d" exit-status))
      (message "%s" (buffer-string)))))
(define-key my-original-map (kbd "p") #'my-prettier-write)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp +n
(use-package emoji-cheat-sheet++ :disabled t
  :load-path "my-lisp"
  )
#+END_SRC
        ...
        (let ((base (event-basic-type ev)))
          (cond ((eq base ?+)
                ...
                (t
                 (setq ev nil))))
        (when ev
          (set-frame-font (format "%s %d" my-default-font my-current-font-size) nil t)
...
#+END_SRC
** Windowsのプロセスの文字コードを設定
[[https://www49.atwiki.jp/ntemacs/pages/16.html][UTF-8 をベースとして利用するための設定 - NTEmacs @ ウィキ - アットウィキ]] から必要なところを拝借して利用。
...
これがないとripgrep.elを使ったときに日本語で検索できなかった。
** ファイル末尾の自動改行挿入をしない
#+BEGIN_SRC emacs-lisp +n
...
~fill-column~ の桁数にインジケーターを表示するマイナーモード。サードパーティライブラリとして ~fill-column-indicator.el~ が存在していたが、Emacs 27から標準ライブラリに入ったようだ。

#+begin_src emacs-lisp +n
(use-package display-fill-column-indicator :defer t
  :ensure nil
  :when (>= emacs-major-version 27)
  :hook ((markdown-mode yaml-mode git-commit-mode)
...
(use-package tab-bar
  ...
  :when (>= emacs-major-version 27)
  :init
  (defvar my-screen-map (make-sparse-keymap) "Keymap for screen key bindings.")
  (defalias 'my-screen-prefix my-screen-map)
  (global-set-key (kbd "C-z") 'my-screen-prefix)
  :bind (("M-n" . tab-next)
         ("M-p" . tab-previous)
         :map my-screen-map
         ("C-c" . tab-new)
         ...
         ("C-p" . tab-previous))
  :config
  (setq tab-bar-close-button-show nil) ; 閉じるボタン非表示
  (setq tab-bar-tab-name-function #'tab-bar-tab-name-truncated) ; 長いタブ名を切り取る
  (tab-bar-mode 1)
  )
#+end_src
** elisp-mode
...
(use-package browse-url :defer t
  ...
  ;; WSL内ではwslviewを使用する
  (when (and (wslp) (executable-find "wslview"))
...
(use-package dired+ :defer t
  ...
  (bind-key "M-p" nil dired-mode-map)
  (bind-key "M-n" nil dired-mode-map)
  (bind-key "C-M-g" nil dired-mode-map)
  )
...
#+BEGIN_SRC emacs-lisp +n
(defun dired-my-open-wslview (file)
  "Try to run `wslview' to open FILE."
  (interactive)
  (if (executable-find "wslview")
      (let ((winpath (wslpath file t 'windows)))
        (call-process "bash" nil nil nil "wslview" winpath))
    nil))
...
(use-package sh-script :defer t
  ...
  (defun my-shfmt-indentation ()
    "shfmtのフォーマットと合うインデント設定にする。"
    (interactive)
    (indent-tabs-mode 1)
    (sh-load-style "shfmt"))
  )
...
(use-package desktop :defer t
  :ensure nil
  :init
  (defun my-turn-on-desktop ()
    "Turn on `desktop-save-mode' and restore buffers."
    (interactive)
    (desktop-save-mode 1)
    (desktop-read))
  :bind ("<f2>" . my-turn-on-desktop)
  :custom
  ...
  :config
  (add-to-list 'desktop-path (my-locate-user-tmp-file "."))
  )
#+END_SRC
** uniquify.el
...
#+end_src
** ivy
現代的なミニバッファの補完インタフェース。helmと違って候補の選択画面に通常のバッファを使わず、ミニバッファなのが良いところでもあり悪いところでもある。

#+begin_src emacs-lisp +n
(use-package ivy :defer t
  :delight
  :init
  (defun ivy-my-height-ratio (ratio)
    "Return a lambda to multiply a frame height by RATIO"
    (lambda (&optional _caller)
      (floor (* (frame-height) ratio))))
  :config
  ...
  ;; `ivy-switch-buffer' のファイル名表示を省略表記にする
  ...
  (setq ivy-wrap t)
  ;; デフォルトの正規表現構築方法を `ivy--regex-ignore-order' にする（順序を無視してマッチする）
  (ivy--alist-set 'ivy-re-builders-alist t #'ivy--regex-ignore-order)
  ;; プロンプトの先頭でDEL (C-h)してもivyを終了させない
...
ivyの候補にall-the-iconsのアイコンをつけてivy-rich-modeと連携してくれる。特にファイルを選択するときに視認性が上がってよろしい。ただしivy-rich-modeは絞り込みやカーソル移動のパフォーマンスが落ちたりするし、アイコンのサイズも完全に揃えようとすると沼るのに注意。
...
(use-package all-the-icons-ivy-rich
  ...
  (use-package ivy-rich
    :config
    ;; ivy-switch-bufferが遅い問題のワークアラウンド
    ;; ref. https://github.com/Yevgnen/ivy-rich/issues/87#issuecomment-740440509
    (progn
  ...
  ;; 遅延ありでall-the-icons-ivy-rich-mode/ivy-rich-modeを有効にする
  (defun ivy-my-enrich ()
    (all-the-icons-ivy-rich-mode 1)
    (ivy-rich-mode 1))
  (run-with-idle-timer 3 nil #'ivy-my-enrich)
  )
#+end_src
*** counsel
ivy関連の便利コマンド集。
...
(use-package counsel :defer t
  ...
  :config
  ;; `counsel-M-x'の初期入力の "^" が邪魔なので空文字に変更する
  (ivy-configure 'counsel-M-x :initial-input "")
  ;; `counsel-find-file'でカーソル下の単語を拾う
  (setq counsel-find-file-at-point t)
  ;; `counsel-yank-pop'の候補間にセパレーターを入れる
  (setq counsel-yank-pop-separator "\n--------------------------------\n")
  ;; 高さ設定
  (cl-loop for (command . height)
           in '((swiper . 0.6)
                (counsel-recentf . 0.5)
                (counsel-rg . 0.6)
                (counsel-yank-pop . 0.5))
           do (ivy-configure command :height (ivy-my-height-ratio height)))
  ;; counsel-rg関連
  (defun counsel-my-rg-toggle-hidden (_arg)
    "Call `counsel-rg' again after toggling \"--hidden\" option."
    (interactive)
    (let* ((opt "--hidden")
           (hidden-on (member opt counsel-rg-base-command))
           (prompt (if hidden-on "--no-hidden" "--hidden")))
      ;; Toggle --hidden option
      (if hidden-on
          (setq counsel-rg-base-command (delete opt counsel-rg-base-command))
        (add-to-list 'counsel-rg-base-command opt :append))
      (counsel-rg ivy-text default-directory nil (format "rg [%s]: " prompt))))
  (defun counsel-my-rg-toggle-ignore (_arg)
    "Call `counsel-rg' again after toggling \"--no-ignore\" option."
    (interactive)
    (let* ((opt "--no-ignore")
           (no-ignore (member opt counsel-rg-base-command))
           (prompt (if no-ignore "--ignore" "--no-ignore")))
      ;; Toggle --no-ignore option
      (if no-ignore
          (setq counsel-rg-base-command (delete opt counsel-rg-base-command))
        (add-to-list 'counsel-rg-base-command opt :append))
      (counsel-rg ivy-text default-directory nil (format "rg [%s]: " prompt))))
  (defun counsel-my-rg-up-directory (_arg)
    "Call `counsel-rg' again after moving up a directory."
    (interactive)
    (let ((dir (ivy--parent-dir default-directory)))
      (counsel-rg ivy-text dir nil (format "rg [%s]: " dir))))
  (ivy-set-actions
   'counsel-rg
   '(("h" counsel-my-rg-toggle-hidden "Toggle hidden")
     ("i" counsel-my-rg-toggle-ignore "Toggle ignore")
     ("u" counsel-my-rg-up-directory "Up directory")))
  ;; counsel-recentfの候補のうちHOME以下のファイルを ~/ で始める
  (advice-add 'counsel-recentf-candidates :filter-return #'my-abbreviate-files)
  (counsel-mode 1))
#+end_src
*** ivy-posframe

#+begin_src emacs-lisp +n
(use-package ivy-posframe :disabled t
  :delight
  :after ivy
  :config
  ...
  (setq ivy-posframe-display-functions-alist
        '((swiper . ivy-display-function-fallback)
          (counsel-ibuffer . ivy-display-function-fallback)
          (t . ivy-posframe-display-at-frame-top-center)))
  (ivy--alist-set 'ivy-posframe-height-alist t 40)
  (setq ivy-posframe-border-width 2) ; ボーダーを太くする
  (ivy-posframe-mode 1))
#+end_src
*** amx
~ido~ ライクな ~M-x~ を提供するライブラリ。だけどamxを単体で使うんじゃなくて、counselと一緒に使うと ~counsel-M-x~ で入力履歴をいい感じに保持しておいてくれる。マニュアルに書いておいておくれ。

- [[https://github.com/abo-abo/swiper/issues/1530]]
  - amxはsmexのフォークみたいです。

#+begin_src emacs-lisp +n
(use-package amx :defer t
  :config
  (setq amx-save-file (my-locate-user-tmp-file "amx-items"))
  (setq amx-history-length 100))
#+end_src
*** consel-ghq
ghqo管理下のリポジトリを開く。なぜかMELPAで公開されていない。

#+begin_src emacs-lisp +n
(use-package counsel-ghq :defer t
  :quelpa (counsel-ghq :repo "windymelt/counsel-ghq" :fetcher github)
  :config
  ;; HOMEを ~/ で省略する
  (advice-add 'counsel-ghq--list-candidates :filter-return #'my-abbreviate-files))
#+end_src
** all-the-icons.el
アイコンフォントをAll in Oneでまとめてくれるパッケージ。必要なフォントも一発でダウンロードできる。

#+begin_src emacs-lisp +n
(use-package all-the-icons :defer 2
  :config
  ;(setq inhibit-compacting-font-caches t)
  ...
  ;; individual icons
  (set-fontset-font t #xe917 "file-icons" nil) ; org icon
  (set-fontset-font t #xe926 "file-icons" nil) ; elisp icon
  (set-fontset-font t #xf00a "github-octicons" nil) ; mark-github icon
  (set-fontset-font t #xf0c9 "github-octicons" nil) ; markdown icon
  )
#+end_src
** [[https://github.com/k-talo/volatile-highlights.el][volatile-highlights.el]]
ペーストやアンドゥした領域をハイライトしてくれる。大きい文字をペーストするとカーソルが迷子になったりするので、視覚的に貼り付けた領域がわかるとミスが起きにくい。
...
#+end_src
** helm-ghq
ghqで管理下のリポジトリにかんたんにアクセスできるようになる。

#+begin_src emacs-lisp +n
(use-package helm-ghq :defer t :disabled t)
#+end_src
** [[https://github.com/iqbalansari/emacs-emojify][emojify]]
テキスト中の絵文字を画像で表示するライブラリ。

Emacs26時点だとカラー絵文字に対応していないので、Symbolaフォントなどで描画した単色の絵文字しか表示できない。これだとせっかくの絵文字のみやすさが損なわれてあんまり嬉しくない。

というわけで絵文字に相当する文字を画像で表示するのがこのライブラリの機能である。 ~絵文字に相当する文字~ として ~:)~ のようなアスキーアート、ユニコードにおける絵文字、 ~:smile:~ のようなGithub形式の絵文字を扱うことができる。

#+begin_src emacs-lisp +n
(use-package emojify :defer t
  :init
  (add-to-list 'minor-mode-alist '(emojify-mode " ☺"))
  :bind (:map mode-specific-map
              ("e" . emojify-insert-emoji))
  :hook
  (text-mode . emojify-mode)
  :config
  (set-variable 'emojify-emoji-styles '(unicode github))
  (set-variable 'emojify-emoji-set "twemoji-v2")
  )
#+end_src

~emojify-insert-emoji~ でGithub形式の絵文字を挿入することもできるので、コミットメッセージの入力などでも使えて便利です。
** [[https://github.com/DarthFennec/highlight-indent-guides][highlight-indent-guides]]
インデント位置をハイライトするマイナーモード。インデント位置はカラムのfaceやcharacterで表現される。yamlなんかで特に便利。
...
(use-package highlight-indent-guides :defer t
  ...
  :config
  (set-variable 'highlight-indent-guides-auto-enabled t)
  (set-variable 'highlight-indent-guides-responsive 'stack)
  (set-variable 'highlight-indent-guides-method 'character)
  (set-variable 'highlight-indent-guides-auto-odd-face-perc 40)
  (set-variable 'highlight-indent-guides-auto-even-face-perc 40)
  (set-variable 'highlight-indent-guides-auto-character-face-perc 40))
#+end_src

~highlight-indent-guides-character~ でインデント位置に表示する文字を設定できる。デフォルトは ~│~ なのだが、これがRictyフォントだと2文字幅になってしまう。そのためインデントがずれてしまって糞の役にも立たなくなる。仕方ないので ~|~ に設定して1文字幅になるようにしている。文字として入力した ~|~ 見分けがつきにくいのが玉に瑕。
** [[https://github.com/dryman/toml-mode.el][toml-mode.el]]
Emacs Mojor mode for editing TOML files。よく設定ファイルに使われてるやつですね。

#+BEGIN_SRC emacs-lisp +n
(use-package toml-mode :defer t)
#+END_SRC
** [[http://emacs.rubikitch.com/beacon/][beacon.el]]
カーソル移動時に一時的に明滅させて、カーソル位置を見失わないようにできる。ウィンドウを移動したりバッファを移動したタイミングで明滅する。

#+BEGIN_SRC emacs-lisp +n
(use-package beacon :defer 10
  :delight
  :custom
  (beacon-size 60)
  :config
...
(use-package editorconfig
  :delight (editorconfig-mode (if (member "file-icons" (font-family-list))
                                  " "
                                " EC"))
  :config
  (editorconfig-mode 1))
...
(use-package atomic-chrome :defer 5
  ...
  (when (fboundp #'my-focus-xwindow-by-ahk)
    ;; WSLのEmacsフレーム（X Server）に自動でフォーカスを当てる
    (add-hook 'atomic-chrome-edit-mode-hook #'my-focus-xwindow-by-ahk))
  )
#+END_SRC
...
#+BEGIN_SRC emacs-lisp +n
(use-package emoji-cheat-sheet-plus :defer t :disabled t
  :init
...
(with-eval-after-load 'flycheck
  (use-package flycheck-inline :disabled t
...
#+END_SRC
*** [[https://github.com/alexmurray/flycheck-posframe][flycheck-posframe.el]]
posframeを使ってメッセージを表示する。
...
(use-package flycheck-posframe
  ...
  :config
  (set-face-background 'flycheck-posframe-background-face "#333333")
  )
#+end_src
** git-gutter-fringe
#+BEGIN_SRC emacs-lisp +n
...
(use-package anzu :defer t
  :delight
  :init
  (add-autoload isearch-forward anzu)
  :config
  (global-anzu-mode)
...
(use-package fill-column-indicator :defer t :disabled t
  :config
...
(use-package open-junk-file :defer t
  ...
  :config
  ;; WSLでもWindows側にジャンクファイルを保存する
  (when (wslp)
    (let ((home (or (getenv "USERPROFILE") (getenv "HOME"))))
      (setq open-junk-file-directory
            (expand-file-name "junk/%Y/%m/%d-%H%M%S." home))))
  )
#+END_SRC
** lispxmp.el
式の評価結果を注釈する．
...
#+END_SRC
*** [[https://github.com/dandavison/magit-delta][magit-delta.el]]
magitのdiffを[[https://github.com/dandavison/delta][delta]]で表示できるパッケージ。diffの可読性がめちゃんこ上がる。
...
#+end_src
*** git-commit
~git commit~ 時のマイナーモード。直接magitに依存はしていないが、magitの依存で入るのでmagit傘下に配置する。
...
(use-package git-commit :defer t
  ...
  ;; fill-columnを80に設定して本文が80文字を超えないようにする
  (add-hook 'git-commit-mode-hook (lambda () (set-fill-column 80))))
#+end_src
*** magit-process
#+begin_src emacs-lisp +n
(use-package magit-process :defer t
  :ensure magit
  :config
  (with-eval-after-load 'emojify
    ;; emojify-modeで絵文字を表示できるようにする
    (add-hook 'magit-process-mode-hook #'emojify-mode)))
#+end_src
** col-highlight.el
...
#+BEGIN_SRC emacs-lisp +n
(use-package helm-config :defer 3 :disabled true
  :ensure helm
  :delight
  (helm-mode)
  (helm-migemo-mode)
...
*** helm-swoop
#+BEGIN_SRC emacs-lisp +n
(use-package helm-swoop :defer t :disabled t
  :init
...
** helm-locate
#+BEGIN_SRC emacs-lisp +n
(use-package helm-locate :defer t :disabled t
  :ensure helm
...
(use-package key-chord
  ...
  (key-chord-mode 1)
  (key-chord-define global-map "nb" 'ivy-switch-buffer)
...
*** helm-buffers
#+BEGIN_SRC emacs-lisp +n
(use-package helm-buffers :defer t :disabled t
  :ensure helm
  :config
...
git管理下のファイルのhelmソース。
#+BEGIN_SRC emacs-lisp +n
(use-package helm-ls-git :defer t :disabled t
  :if (executable-find "git")
  :init
...
#+END_SRC
**** 見出しのIDをランダムではなくパスに基づく値にする
デフォルトはランダムなので、生成するたびに異なるIDになってしまう。見出しのタイトルや深さにから算出したチェックサムをIDにすることで、複数回生成しても同じIDになるようにする。

#+begin_src emacs-lisp +n
(with-eval-after-load 'ox-html
  (defun my-org-element-headline-path (element)
    "Return a path of headline if ELEMENT type is headline."
    (when (eq (org-element-type element) 'headline)
      (let* ((current-head element)
             (path (concat "/" (org-element-property :raw-value current-head))))
        (while (org-export-get-parent-headline current-head)
          (setq current-head (org-export-get-parent-headline current-head))
          (setq path (concat "/" (org-element-property :raw-value current-head) path)))
        path)))
  (defun my-org-headline-reference (datum info)
    (let ((cache (plist-get info :internal-references))
          (ast (plist-get info :parse-tree))
          new-cache)
      (unless cache
        (message "`my-org-headline-reference': Cache not found")
        (org-element-map ast 'headline
          (lambda (headline)
            (let* ((custom-id (org-element-property :CUSTOM_ID headline))
              ...
              (setq headline-ref
                    (or custom-id
                        (format "org-%s" (md5 (my-org-element-headline-path headline)))))
              ;; put a headline refernce into cache
              (push (cons headline-ref headline) new-cache))))
        (plist-put info :internal-references new-cache))))
  (advice-add #'org-export-get-reference :before #'my-org-headline-reference)
  )
#+end_src
...
#+BEGIN_SRC emacs-lisp +n
(use-package ox-reveal :defer t :disabled t
  :init
...
<h3 class=\"date\">%d</h3>")
  ...
  (defun org-reveal-open-new-link-filter (data backend info)
    ...
    (when (memq backend '(reveal re-reveal))
...
org-revealのフォーク。こちらのほうが活発そうなので、こちらを使っていこう。

#+begin_src emacs-lisp +n
(use-package org-re-reveal :defer t
  :init
  (add-autoload org-export-dispatch ox-re-reveal)
  :config
  (setq org-re-reveal-title-slide "<h1 class=\"title\">%t</h1>
...
(use-package ox-gfm :defer t
  :init
  (add-autoload org-export-dispatch ox-gfm)
  :config
  (setq org-pandoc-options-for-revealjs '((variable . "theme:default"))) ; reveal.jsのデフォルトテーマを利用
  )
#+END_SRC
...
#+END_SRC
** mozc.el
Mozcを使って日本語入力するためのパッケージ。基本的にはWSLのEmacsをVcXsrvで表示して、そこからWindowsのGoogle日本語入力を使用するための設定。
...
- [[https://www49.atwiki.jp/ntemacs/pages/48.html][emacs-mozc を動かすための設定（emacs 設定編） - NTEmacs @ ウィキ - アットウィキ]]
- [[https://www49.atwiki.jp/ntemacs/pages/61.html][emacs-mozc を動かすための設定（WSL 設定編） - NTEmacs @ ウィキ - アットウィキ]]

から必要な設定を抜粋している。具体的な作業としては

- [[https://github.com/smzht/mozc_emacs_helper]] から ~mozc_emacs_helper.exe~ をダウンロードしてきて、Windows側のPATHが通ったところへ配置する。
- 上記の設定を有効にする。

でOKなはず。Windows側のGoogle日本語入力の設定や辞書が有効になり、変換はうまくできていそうだ。。ただ ~全角半角~ キーでのトグル時に何回もトグルが起こってしまうようで、うまくトグルできないときもある。上記のNTEmacs@ウィキに半角全角キーをうまく使えるようにするpythonスクリプトあるのでそれを入れれば改善するかも。

他にもいくつか問題がある。IMを使っているので同じくIMを使っている ~key-chord~ の同時押しキーバインドが使えない。まあこれはWindowsでも日本語入力時は ~key-chord~ の同時押しできてなかったので、まあ同じようなもんか。
*** [[https://github.com/derui/mozc-posframe][mozc-posframe]].el
mozcの変換候補をposframeで表示する。

#+begin_src emacs-lisp +n
(use-package mozc-posframe
  :quelpa (mozc-posframe :repo "derui/mozc-posframe" :fetcher github)
  :if (linuxp)
  :after mozc
  :config
...
#+BEGIN_SRC emacs-lisp +n
  (use-package migemo
    ...
    (defun migemo-dict-find ()
      ...
      (cl-loop for dict
               in (list
                   (expand-file-name "utf-8/migemo-dict" "/usr/share/cmigemo")
