...
(when (linuxp)
  ...
  (use-package mozc-im
    ...
    ;; IME有効時のタイトル設定
    (let ((icon (expand-file-name "images/mozc-ui-hiragana.png" user-emacs-directory)))
      (if (file-exists-p icon)
          (setq mozc-leim-title (propertize "あ" 'display `(image :type png :file ,icon :ascent center :margin (2 . 0))))
